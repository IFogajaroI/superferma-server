...
LOG = Path("superferma_v44_mow_isolated_test.log")
PRINT_LIMIT = 65535
...
def tcp_client(conn, addr):
    ...
    # Isolated ACTION 202 test state. This is intentionally per TCP session so
    # every game restart starts clean without changing the static UserState.
    mowed_fields = set()
    mow_grass_count = 0
        ...
        while True:
            ...
            if action == 202:
                ...
                if field_id is None:
                    ...
                    duplicate = field_id in mowed_fields
                    if not duplicate:
                        mowed_fields.add(field_id)
                        mow_grass_count += 1

                    # IMPORTANT: v43 used status="grow", which could immediately
                    # put the tile back into the grass-growth state. For this isolated
                    # test we use only the already-confirmed FarmField status "digged".
                    # This should remove the grass visually and make the same tile no
                    # longer a valid grass target.
                    farm_field = (
                        fld_varint(1, field_id) +
                        fld_bytes(6, b"digged")
                    )

                    # Retain the mower resource update attempted in v43, but with
                    # valid protobuf bytes and a stable cumulative count.
                    # MowMachineProduct: field 1 = product_id (grass=0), field 2 = count.
                    mow_product = (
                        fld_varint(1, 0) +
                    ...
                    log(
                        f'*** ACTION 202: field={field_id}, mower={mm_id}, '
                        f'duplicate={duplicate}, grass_count={mow_grass_count}, '
                        'farm_field.status="digged" ***'
                    )
                    log(f"*** ACTION 202 MOWED_FIELDS={sorted(mowed_fields)} ***")

                send_normal_response(
                    conn,
                    mow_payload,
                    "Action202 Isolated Mow FarmField+MowMachine Response + 00 envelope"
                )
                log("*** ACTION 202 -> isolated mow state SENT ***")
                continue
...
def tcp_server():
    ...
    s.listen(30)
    log("TCP v44-mow-isolated-test listening on 127.0.0.1:8866")
...
def http_server():
    ...
    log("HTTP v44-mow-isolated-test listening on 127.0.0.1:8080")
    srv.serve_forever()

if __name__ == "__main__":
    print("Superferma local server v44-mow-isolated-test")
    print("----------------------------------------------")
    print("IMPORTANT:")
    print("  NO memory patches or probes for this test.")
    print("  Restart game.exe before testing v44.")
    print("")
    ...
    print('  ACTION 202 -> FarmField(status="digged") + MowMachine grass count [isolated test]')
    ...
    print("Send superferma_v44_mow_isolated_test.log + screenshot if anything changes.")
    print("")
