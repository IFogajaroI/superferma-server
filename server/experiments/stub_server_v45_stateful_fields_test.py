...
LOG = Path("superferma_v45_stateful_fields_test.log")
PRINT_LIMIT = 65535
...
def build_farm_field(field_id, state):
    # common.proto FarmField field order recovered from vf.dll:
    #   1=id, 2=water_amount, 4=product_id, 5=percent_growth, 6=status
    # Fields 1/2/6 are directly confirmed. Fields 4/5 are high-confidence
    # schema mappings and are intentionally isolated in this v45 experiment.
    return (
...
def tcp_client(conn, addr):
    ...
    def field_state(field_id):
        return farm_fields.setdefault(field_id, {
            "status": "grow",
            "water_amount": 0.0,
            "product_id": 0,
            "percent_growth": 0.0,
        ...
        while True:
            ...
            if action == 203:
                ...
                if field_id is None:
                    ...
                    before = dict(state)
                    state["water_amount"] = 1.0
                    water_payload = response_with_farm_field(field_id, state)
                    log(f"*** ACTION 203: field={field_id} WATER before={before} after={state} ***")
                send_normal_response(conn, water_payload, "Action203 Stateful Water FarmField Response + 00 envelope")
                log("*** ACTION 203 -> full FarmField with water_amount=1.0 SENT ***")
                continue
            ...
            if action == 201:
                ...
                if field_id is None:
                    ...
                    state = field_state(field_id)
                    before = dict(state)
                    state.update({"status": "digged", "water_amount": 0.0, "product_id": 0, "percent_growth": 0.0})
                    last_mow_at.pop(field_id, None)
                    dig_payload = response_with_farm_field(field_id, state)
                    log(f"*** ACTION 201: field={field_id} DIG before={before} after={state} ***")
                send_normal_response(conn, dig_payload, "Action201 Stateful Dig FarmField Response + 00 envelope")
                log("*** ACTION 201 -> full FarmField(status=digged, water=0) SENT ***")
                continue
            ...
            if action == 202:
                ...
                if field_id is None:
                    ...
                    farm_field = build_farm_field(field_id, state)
                    mow_product = fld_varint(1, 0) + fld_varint(2, mow_grass_count)
                    mm_id = mow_machine_id if mow_machine_id is not None else 2
                    mow_machine = fld_varint(1, mm_id) + fld_varint(2, 0) + fld_bytes(5, mow_product)
                    mow_payload = fld_varint(1, 1) + fld_bytes(3, farm_field) + fld_bytes(6, mow_machine)
                    log(f"*** ACTION 202: field={field_id} MOW mower={mm_id} duplicate={duplicate} grass_count={mow_grass_count} before={before} after={state} ***")
                send_normal_response(conn, mow_payload, "Action202 Stateful Mow FarmField+MowMachine Response + 00 envelope")
            ...
            if action == 204:
                ...
                if field_id is None:
                    ...
                    log(f"*** ACTION 204: field={field_id} SOW seed_packet={seed_packet_id} product_id={product_id} before={before} after={state} ***")
                send_normal_response(conn, sow_payload, "Action204 Stateful Sow FarmField Response + 00 envelope")
            ...
            if action == 5:
                ...
                log(f"*** ACTION 5 -> seed_packet_id=1 mapped to product_id={product_id}; seed_packets={seed_packets} ***")
                continue
            ...
            send_normal_response(conn, fallback_payload, f"Action{action} Generic Response + 00 envelope")
...
def tcp_server():
    ...
    s.listen(30)
    log("TCP v45-stateful-fields-test listening on 127.0.0.1:8866")
    while True:
        c, a = s.accept()
...
def http_server():
    ...
    log("HTTP v45-stateful-fields-test listening on 127.0.0.1:8080")
    srv.serve_forever()

if __name__ == "__main__":
    print("Superferma local server v45-stateful-fields-test")
    print("----------------------------------------------")
    print("IMPORTANT:")
    print("  NO memory patches or probes for this test.")
    print("  Restart game.exe before testing v45.")
    print("")
    ...
    print("  ACTION 203 -> full FarmField, water_amount=1.0, other state preserved")
    ...
    print('  ACTION 201 -> full FarmField(status="digged", water=0, product=0)')
    ...
    print("Send superferma_v45_stateful_fields_test.log + screenshot if anything changes.")
    print("")
