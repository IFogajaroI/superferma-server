...
LOG = Path("superferma_v46_chudo_growth_test.log")
PRINT_LIMIT = 65535
...
def tcp_client(conn, addr):
    ...
    try:
        while True:
            ...
            if action == 106:
                # Experimental heartbeat sync: if we have a currently growing
                # product_id=1 field, return its latest FarmField snapshot.
                # This gives the client a chance to observe the 30+30+30 timer
                # without an unsolicited server push.
                if last_growth_field_id is not None and last_growth_field_id in farm_fields:
                    ...
                    send_normal_response(
                        conn, action106_payload,
                        "Action106 Growth FarmField Response + 00 envelope"
                    )
                else:
            ...
            if action == 201:
                ...
                if field_id is None:
                    ...
                    state = field_state(field_id)
                    before = dict(state)
                    # Digging resets the cell to a clean plantable state.
                    state.update({
                        "status": "digged",
                        "water_amount": 0.0,
                        "product_id": 0,
...
def tcp_server():
    ...
    s.listen(30)
    log("TCP v46-chudo-growth-test listening on 127.0.0.1:8866")
...
def http_server():
    ...
    log("HTTP v46-chudo-growth-test listening on 127.0.0.1:8080")
    srv.serve_forever()

if __name__ == "__main__":
    print("Superferma local server v46-chudo-growth-test")
    print("----------------------------------------------")
    print("IMPORTANT:")
    print("  NO memory patches or probes for this test.")
    print("  Restart game.exe before testing v46.")
    print("")
    ...
    print("  ACTION 106 heartbeat re-sends latest timed FarmField percent_growth")
    print("")
    ...
    print("Send superferma_v46_chudo_growth_test.log + screenshot after the growth test.")
    print("")
